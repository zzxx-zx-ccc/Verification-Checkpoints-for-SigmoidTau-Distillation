...
class Model(nn.Module):
    ...
    def train(
        ...
        **kwargs,
        ...
        # Keep resume handling compatible with distillation training.
        # if args.get("resume"):
        #     args["resume"] = self.ckpt_path

        self.trainer = (trainer or self._smart_load("trainer"))(overrides=args, _callbacks=self.callbacks)
        if not args.get("resume"):  # manually set model only if not resuming
            # Load weights correctly for yaml-defined distillation models.
            self.trainer.model = self.trainer.get_model(weights=self.model if (self.ckpt or bool_distill) else None, cfg=self.model.yaml)
